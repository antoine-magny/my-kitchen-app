...
function FilterChip({
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-bold whitespace-nowrap transition-all duration-200"
      style={{
        background: active ? (favoris ? "#E85D75" : "#1C2B1E") : "#FFFFFF",
        color: active ? "#FFFFFF" : favoris ? "#E85D75" : "#4A7C59",
        border: active
          ? `1.5px solid ${favoris ? "#E85D75" : "#1C2B1E"}`
          : favoris
            ? "1.5px solid #F9C5CF"
            : "1.5px solid #C8E0CF",
        boxShadow: active
          ? `0 4px 12px ${favoris ? "rgba(232,93,117,0.25)" : "rgba(28,43,30,0.18)"}`
          : "0 1px 4px rgba(28,43,30,0.05)",
      }}
    >
      {favoris ? (
