...
import { formatTodayLongFr, parisCalendarDate } from "@/lib/date-paris";
import {
  daysUntilDlc,
  dlcStatus,
            ...
            {todayMeal?.recipe ? (
              <>
                <div className="relative h-52 bg-[#D4EDD9]">
                  <Link href={`/recettes/${todayMeal.recipe.id}`} className="absolute inset-0 block">
                    {todayMeal.recipe.photo ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={todayMeal.recipe.photo} alt={todayMeal.recipe.title} className="h-full w-full object-cover" />
                    ) : (
                  ...
                  >
                    ...
                    <span className="text-xs font-bold text-[#1C2B1E]">{todayMeal.recipe.time}</span>
                  </div>
                </div>
                ...
                <div className="px-5 py-4">
                  <div className="mb-2 flex items-center gap-1.5">
                    ...
                    <span className="ml-1 text-xs font-medium text-[#7A8F7D]">
                      4.2 • {todayMeal.recipe.servings} pers.
                    </span>
                  </div>

                  <Link href={`/recettes/${todayMeal.recipe.id}`}>
                    <h3 className="font-lora mb-4 text-base leading-snug font-bold text-[#1C2B1E]">{todayMeal.recipe.title}</h3>
                  </Link>
                  ...
                  <div className="flex items-center gap-2.5">
                    <div className="flex items-center gap-1.5 rounded-xl bg-[#FFF7ED] px-3 py-1.5">
                      ...
                      <span className="text-xs font-bold text-[#C2410C]">{todayMeal.recipe.calories} kcal</span>
                    </div>
                    <div className="flex items-center gap-1.5 rounded-xl bg-[#EBF2EC] px-3 py-1.5">
                      <span className="text-[#4A7C59]">
                        <ProteinIcon size={13} />
                      </span>
                      <span className="text-xs font-bold text-[#2E5C3A]">{todayMeal.recipe.proteins}g protéines</span>
                    </div>
                  </div>

                  <Link
                    href={`/recettes/${todayMeal.recipe.id}`}
